...
function EnhancedTableHead(props) {
  ...
  const [alert, setAlert] = useState({open: false, backgroundColor: '#ff3232', message: "Row deleted!"})
...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, rows, setRows, setSelected, filterPrice, setFilterPrice, totalFilter, setTotalFilter } = props;
  ...
  const handleTotalFilter = (event) => {
    ...
    if(event.target.value !== ""){
      const newRows = [...rows];
      newRows.map(row => eval( `${event.target.value} ${totalFilter === "="? "===" : totalFilter} ${row.total.slice(1, row.total.length)}`) ? row.search = true : row.search = false);
      setRows(newRows);
    }
  }
  ...
  const filterChange = (operator) => {
    if(filterPrice !== ""){
      const newRows = [...rows];
      newRows.map(row => eval( `${filterPrice} ${operator === "="? "===" : operator} ${row.total.slice(1, row.total.length)}`) ? row.search = true : row.search = false);
      setRows(newRows);
    }else{
      const newRows = [...rows];
    ...
    >
      ...
      <Snackbar
        ...
        onClose={(event, reason) => {
          if(reason === 'clickaway'){
            setAlert({...areIntervalsOverlappingWithOptions, open: false}); 
            const newRows = [...rows];
        ...
        action={
          <Button style={{color: "#fff"}} onClick={onUndo}>Undo</Button>
        }
      />
      <Menu 
          id="services-menu" 
...
export default function EnhancedTable({ rows, setRows, page, setPage, websiteChecked, iOSChecked, androidChecked, softwareChecked }) {
  ...
  const switchFilters = () => {
    ...
    const softwareApps = rows.filter(row => softwareChecked ? row.service === "Custom Software" : null);
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper} evelavtion={0}>
